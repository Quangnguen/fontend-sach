...
export const add_friend = createAsyncThunk(
    ...
    async(info, { rejectWithValue,fulfillWithValue }) => {
        try {
            const {data} = await api.post('/chat/customer/add-customer',info)
  ...
  extraReducers: (builder) => {
    builder
      .addCase(add_friend.fulfilled, (state, { payload }) => {
        state.fb_messages = payload.data.messages
        state.currentFd = payload.data.currentFd
        state.my_friends = payload.data.MyFriends
      })

      .addCase(send_message.fulfilled, (state, { payload }) => {
        let tempFriends = state.my_friends
        let index = tempFriends.findIndex(
          (f) => f.fdId === payload.data.message.receverId
        )
        while (index > 0) {
        ...
        state.fb_messages = [...state.fb_messages, payload.data.message]
